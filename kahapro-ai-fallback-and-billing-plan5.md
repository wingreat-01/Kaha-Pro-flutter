...
## Part 1 — 6-Provider AI Fallback (Groq → Mistral → Gemini → DeepSeek → OpenRouter → OpenAI 💰)
...
Chosen so five free/cheap-tier providers get exhausted (in order of speed/generosity) before ever touching OpenAI, the confirmed tier-6 paid provider. Order rationale:
...
- **Gemini** third — best free-tier *quality* (frontier-tier model) but tightest daily caps (as low as 100 RPD on 2.5 Pro)
- **DeepSeek** fourth — 5M free tokens/month with no card, then extremely cheap beyond that (~$0.14/$0.28 per M tokens on V4 Flash); positioned after the pure-free tiers rather than first because its direct API is hosted primarily in China and can hit capacity constraints at peak demand — fine as a mid-chain fallback, riskier as a primary
- **OpenRouter** fifth — aggregator; last free-tier safety net across many free community models before paying
- **OpenAI (paid)** last — only reached if all five earlier tiers are exhausted; at KahaPro's current volume (50 Pro stores × 30 credits = 1,500 requests/mo worst case, even if *all* hit tier 6) this costs roughly **$1–8/month** depending on prompt size, so it's not a meaningful cost concern yet — chosen over DeepSeek here for official Western-hosted infra, no peak-capacity risk, and mature tooling, since this is the "must basically always work" tier. Revisit only if usage scales into the thousands of stores or per-request token size turns out much heavier than estimated.

**Cerebras was evaluated and dropped** — as of Aug 17, 2026, its free tier requires a payment method on file (confirmed via a real error hit in UpaPro), which defeats the point of a zero-friction free-tier fallback.
...
3. Attempt the AI call chain (Groq → Mistral → Gemini → DeepSeek → OpenRouter → OpenAI)
...
Each `callGroq` / `callMistral` / `callGemini` / `callDeepSeek` / `callOpenRouter` / `callOpenAI` function:
...
- API keys pulled from Supabase secrets (`GROQ_API_KEY`, `MISTRAL_API_KEY`, `GEMINI_API_KEY`, `DEEPSEEK_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_API_KEY`), never hardcoded

### Provider free/cheap-tier reference (as of testing, verify before relying on for production)
| Provider | Free/cheap limit | Notes |
|---|---|---|
...
| Gemini | Flash: 15 RPM/1,500 RPD · 2.5 Pro: 5 RPM/100 RPD | Best raw quality of the free tiers, but tightest daily cap |
| DeepSeek | 5M free tokens/mo, no card; then ~$0.14/$0.28 per M tokens (V4 Flash) | ⚠️ Direct API hosted primarily in China, can hit peak-demand capacity constraints — acceptable mid-chain, not as a primary |
...
| OpenAI | Paid, no free cap (~$1–8/mo at current 50-store volume) | Only hit if all 5 earlier tiers fail — needs its own cost monitoring/ceiling |

### Open decisions
- [x] Exact model names confirmed: `llama-3.3-70b-versatile` (Groq), `mistral-small-latest` (Mistral), `gemini-2.5-flash` (Gemini), `deepseek-v4-flash` (DeepSeek), `openrouter/free` (OpenRouter — a built-in random router across all 18 current free-variant models, avoiding the single-model catalog-drift risk entirely), `gpt-5.4-nano` (OpenAI)
- [ ] Decide whether `provider` used should be logged per request (useful for cost tracking / debugging which provider is flaky, and for spotting when OpenAI is getting hit too often)
...
- [ ] DeepSeek peak-demand capacity — decide on retry/skip behavior if it times out rather than errors cleanly
- [ ] Set a cost ceiling/alert on OpenAI usage, since it's the only tier that bills per request (revisit if volume scales well beyond 50 Pro stores)

### Testing checklist
- [ ] Force Groq to fail (bad API key temporarily) → confirm Mistral picks up
- [ ] Force Groq + Mistral to fail → confirm Gemini picks up
- [ ] Force Groq + Mistral + Gemini to fail → confirm DeepSeek picks up
- [ ] Force all four earlier providers to fail → confirm OpenRouter picks up
- [ ] Force all five earlier providers to fail → confirm OpenAI picks up, and that this event is logged distinctly (it costs money)
- [ ] Force all six to fail → confirm clean "AI temporarily unavailable" response to Flutter (not a raw 500), and no credit deducted
...
| 6-provider AI fallback (Groq → Mistral → Gemini → DeepSeek → OpenRouter → OpenAI) | 📋 This doc, Part 1 |
