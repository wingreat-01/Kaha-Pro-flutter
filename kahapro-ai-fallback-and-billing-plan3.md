...
## Part 1 — 6-Provider AI Fallback (Groq → Cerebras → Mistral → Gemini → OpenRouter → OpenAI 💰)
...
Chosen so five free-tier providers get exhausted (in order of speed/generosity) before ever touching the paid premium provider — **OpenAI is now the confirmed tier-6 premium provider**, closing the earlier open decision. Order rationale:
- **Groq** first — fastest response time, generous free RPM, best default UX
- **Cerebras** second — very high throughput when its free catalog has a usable model, but catalog is volatile, so it's positioned early to take advantage of its speed while it's available, with fallbacks ready if a model gets deprecated
- **Mistral** third — solid quality, prototyping-friendly free tier
- **Gemini** fourth — best free-tier *quality* (frontier-tier model) but tightest daily caps (as low as 100 RPD on 2.5 Pro)
- **OpenRouter** fifth — aggregator; last free-tier safety net across many free community models before paying
- **OpenAI (paid)** last — only reached if all five free tiers are exhausted; costs real money per request, so should get logged/alerted distinctly from the free-tier fallbacks, and needs a cost ceiling
...
3. Attempt the AI call chain (Groq → Cerebras → Mistral → Gemini → OpenRouter → OpenAI)
...
async function callWithFallback(prompt: string): Promise<ProviderResult> {
  const providers = [
    ...
    { name: 'openai', fn: callOpenAI }, // paid — only reached if all 5 free tiers fail
  ];
...
Each `callGroq` / `callCerebras` / `callMistral` / `callGemini` / `callOpenRouter` / `callOpenAI` function:
...
- API keys pulled from Supabase secrets (`GROQ_API_KEY`, `CEREBRAS_API_KEY`, `MISTRAL_API_KEY`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_API_KEY`), never hardcoded
...
| OpenAI | Paid, no free cap | Only hit if all 5 free tiers fail — needs its own cost monitoring/ceiling |

### Open decisions
- [ ] Confirm the exact model names/endpoints per provider (e.g. `llama-3.3-70b-versatile` on Groq, current Cerebras free model, `mistral-small-latest`, `gemini-2.5-flash`, an OpenRouter `:free` model, an OpenAI model like `gpt-4o-mini`) — reuse UpaPro's Agent Ria picks where they overlap
...
- [ ] Cerebras catalog volatility — decide on a fallback behavior if its currently-configured free model gets deprecated (skip straight to Mistral?)
- [ ] Set a cost ceiling/alert on OpenAI usage, since it's the only tier that bills per request

### Testing checklist
- [ ] Force Groq to fail (bad API key temporarily) → confirm Cerebras picks up
- [ ] Force Groq + Cerebras to fail → confirm Mistral picks up
- [ ] Force Groq + Cerebras + Mistral to fail → confirm Gemini picks up
- [ ] Force all four earlier providers to fail → confirm OpenRouter picks up
- [ ] Force all five free providers to fail → confirm OpenAI picks up, and that this event is logged distinctly (it costs money)
...
| 6-provider AI fallback (Groq → Cerebras → Mistral → Gemini → OpenRouter → OpenAI) | 📋 This doc, Part 1 |
