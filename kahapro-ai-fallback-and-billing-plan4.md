...
### Open decisions
- [ ] Confirm the exact model names/endpoints per provider (e.g. `llama-3.3-70b-versatile` on Groq, `mistral-small-latest`, `gemini-2.5-flash`, `deepseek-v4-flash`, an OpenRouter `:free` model, an OpenAI model like `gpt-5.4-nano` or `gpt-5.6-luna`) — reuse UpaPro's Agent Ria picks where they overlap
